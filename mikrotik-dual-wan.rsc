...
# Aggressive Cleanup: Remove old Monitor IP Bypass rules (which might have no comments)
:do { /ip firewall mangle remove [/ip firewall mangle find where action="return" && chain="prerouting"] } on-error={}
:do { /ip firewall mangle remove [/ip firewall mangle find where comment~"PCC" || comment~"Route" || comment~"ISP" || comment~"Return" || comment~"Output" || comment~"Bypass" || comment~"Local:" || action="mark-connection" || action="mark-routing" || action="change-mss"] } on-error={}
...
# BUG FIX: Removed mark-routing on WAN in-interface. Applying routing marks to incoming WAN
# traffic breaks local routing (port forwarding/return traffic) because the ISP routing 
# table lacks connected routes to the LAN. Connection tracking naturally handles return traffic.

# Mark Routing based on Connection Mark (CRITICAL: Keep enabled during failover)
add chain=prerouting connection-mark=ISP1_conn in-interface=$lLANInterface \
    ...
    comment="Route-Mark: LAN-ISP1 (KEEP)"
...
add chain=prerouting connection-mark=ISP2_conn in-interface=$lLANInterface \
    ...
    comment="Route-Mark: LAN-ISP2 (KEEP)"
...
add chain=output src-address=$lWAN1Address action=mark-routing new-routing-mark=ISP1 passthrough=no comment="Output: Force WAN1 Source to ISP1 (Monitor)"
add chain=output src-address=$lWAN2Address action=mark-routing new-routing-mark=ISP2 passthrough=no comment="Output: Force WAN2 Source to ISP2 (Monitor)"
...
add chain=forward protocol=tcp tcp-flags=syn tcp-mss=1401-65535 action=change-mss new-mss=1400 passthrough=yes comment="BS: Clamp MSS to 1400 (Safe for PPPoE/LTE)"
...
# ICMP Rate Limiting (LAN Only)
add chain=input in-interface=$lLANInterface protocol=icmp limit=10,5:packet action=accept comment="Limit: ICMP (LAN Only)"
add chain=input in-interface=$lLANInterface protocol=icmp action=drop comment="Drop: Excess ICMP (LAN)"
...
# NOTE: For port forwarding, add: chain=forward in-interface-list=WAN connection-state=new connection-nat-state=dstnat action=accept
add chain=forward in-interface=$lLANInterface connection-state=new action=accept comment="Accept: LAN Forward (new only, est/rel handled above)"
...
add dst-address=0.0.0.0/0 gateway=$lWAN1Gateway distance=250 comment="FAILSAFE-WAN1" disabled=no
add dst-address=0.0.0.0/0 gateway=$lWAN2Gateway distance=251 comment="FAILSAFE-WAN2" disabled=no
            ...
            :if (\$isp1BothDown) do={
                ...
                :if (\$ISP1Status = \"up\") do={
                    ...
                    :if (\$ISP1FailCount >= \$FailureThreshold) do={
                        ...
                        :delay 500ms; # Stabilization delay for connection state sync
                        
                        # Refactored: Explicit loop for reliability with Crash Protection
                        # 1. Explicitly kill the default route first (Priority)
                        :log warning \"[FAILOVER] Disabling Main ISP1 Route...\"
                        :do { /ip route disable [find where comment~\"Default: ISP1\"] } on-error={ :log error \"[FAILOVER] Failed to disable Default Route!\" }
                        
                        :foreach i in=[/ip route find where comment~\"ISP1\"] do={
                            :do {
                                :local c [/ip route get \$i comment]
                                :local isMonitor ([:typeof [:find \$c \"Monitor\"]] != \"nil\")
                                :local isFailover ([:typeof [:find \$c \"Failover\"]] != \"nil\")
                                :local isFailsafe ([:typeof [:find \$c \"FAILSAFE\"]] != \"nil\")
                                :if (\$isMonitor = false && \$isFailover = false && \$isFailsafe = false) do={
                                    /ip route disable \$i
                                }
                            } on-error={ :log error \"[FAILOVER] Failed to disable route loop\" }
                        }
                        # Only disable PCC classification rules, NOT return/route-mark rules
                        :foreach i in=[/ip firewall mangle find where comment~\"PCC: ISP1\"] do={
                            :do { /ip firewall mangle disable \$i } on-error={}
                        }
                    }
                }
                ...
                :if (\$ISP1Status = \"down\") do={
                    :local wasLongOutage (\$ISP1FailCount >= \$tick1)
                    :set ISP1Status \"up\"; :set ISP1FailCount 0
                    :log info \"[FAILOVER] ISP1 UP - Recovered\"
                    :log warning \"[FAILOVER] Enabling Main ISP1 Route...\"
                    :do { /ip route enable [find where comment~\"Default: ISP1\"] } on-error={ :log error \"[FAILOVER] Failed to enable Default Route!\" }
                    :foreach i in=[/ip route find where comment~\"ISP1\"] do={
                        :do {
                            :local c [/ip route get \$i comment]
                            :local isFailsafe ([:typeof [:find \$c \"FAILSAFE\"]] != \"nil\")
                            :if (\$isFailsafe = false) do={ /ip route enable \$i }
                        } on-error={ :log error \"[FAILOVER] Failed to enable route loop\" }
                    }
                    :foreach i in=[/ip firewall mangle find where comment~\"PCC: ISP1\"] do={ /ip firewall mangle enable \$i }
                    :if (\$EmailEnable = true && \$wasLongOutage) do={
                        :do { /tool e-mail send to=\$EmailTo subject=\"[DualWAN] ISP1 Recovered\" body=\"ISP1 is back online.\" } on-error={ :log error \"[FAILOVER] Recovery Email Failed\" }
                    }
            ...
            :if (\$isp2BothDown) do={
                ...
                :if (\$ISP2Status = \"up\") do={
                    ...
                    :if (\$ISP2FailCount >= \$FailureThreshold) do={
                        ...
                        # Refactored: Explicit loop for reliability
                        # 1. Explicitly kill the default route first (Priority)
                        :log warning \"[FAILOVER] Disabling Main ISP2 Route...\"
                        :do { /ip route disable [find where comment~\"Default: ISP2\"] } on-error={ :log error \"[FAILOVER] Failed to disable Default Route!\" }

                        :foreach i in=[/ip route find where comment~\"ISP2\"] do={
                            :do {
                                :local c [/ip route get \$i comment]
                                :local isMonitor ([:typeof [:find \$c \"Monitor\"]] != \"nil\")
                                :local isFailover ([:typeof [:find \$c \"Failover\"]] != \"nil\")
                        ...
                        # Only disable PCC classification rules, NOT return/route-mark rules
                        :foreach i in=[/ip firewall mangle find where comment~\"PCC: ISP2\"] do={
                            :do { /ip firewall mangle disable \$i } on-error={}
                        }
                    }
                }
                ...
                :if (\$ISP2Status = \"down\") do={
                    :local wasLongOutage (\$ISP2FailCount >= \$tick1)
                    :set ISP2Status \"up\"; :set ISP2FailCount 0
                    :log info \"[FAILOVER] ISP2 UP - Recovered\"
                    :log warning \"[FAILOVER] Enabling Main ISP2 Route...\"
                    :do { /ip route enable [find where comment~\"Default: ISP2\"] } on-error={ :log error \"[FAILOVER] Failed to enable Default Route!\" }
                    :foreach i in=[/ip route find where comment~\"ISP2\"] do={
                        :do {
                            :local c [/ip route get \$i comment]
                            :local isFailsafe ([:typeof [:find \$c \"FAILSAFE\"]] != \"nil\")
                    ...
                    :foreach i in=[/ip firewall mangle find where comment~\"PCC: ISP2\"] do={ /ip firewall mangle enable \$i }
                    :if (\$EmailEnable = true && \$wasLongOutage) do={
                        :do { /tool e-mail send to=\$EmailTo subject=\"[DualWAN] ISP2 Recovered\" body=\"ISP2 is back online.\" } on-error={ :log error \"[FAILOVER] Recovery Email Failed\" }
                    }
                } else={ :set ISP2FailCount 0 }
            }

            :if (\$ISP1Status = \"down\" && \$ISP2Status = \"down\") do={
                :log error \"[FAILOVER] CRITICAL: Both ISPs DOWN - No internet connectivity!\"
            }
        }
    } on-error={ :log error \"[FAILOVER] Script CRASHED\" }
"
...
# Logging Cleanup (Fix for previous duplicate logging)
/system logging
:do {
    ...
    :foreach i in=[/system logging find where topics~"firewall"] do={
        :if ([/system logging get $i prefix] = "[FW] ") do={ /system logging remove $i }
    }
} on-error={}
